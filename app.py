...
def extract_playlist_id(url: str) -> Optional[str]:
    # erlaubt Voll-URL ODER nackte ID
...
def root():
    return (
        "OK: Spotify Cards Backend läuft.\n\n"
        "Nutzung:\n"
        "  /api/playlist.json?url=<spotify_playlist_url_oder_id>&market=DE\n"
        "  /api/playlist.csv?url=<spotify_playlist_url_oder_id>&market=DE\n"
        "  /api/playlist.test?url=<spotify_playlist_url_oder_id>\n"
    )

# Debug-Helfer: zeigt, welche Playlist-ID erkannt wird
@app.get("/api/playlist.test", response_class=PlainTextResponse)
def api_playlist_test(url: str = Query(..., description="Playlist URL oder ID")):
    pid = extract_playlist_id(url)
    return f"parsed playlist_id = {pid or '(keine)'}"

@app.get("/api/playlist.json")
async def api_playlist_json(
    url: str = Query(..., description="Spotify Playlist URL ODER nur ID"),
    market: str = Query("DE", description="Market, z.B. DE/US/GB")
):
    pid = extract_playlist_id(url)
    if not pid:
        raise HTTPException(status_code=400, detail="Ungültige Playlist-URL oder ID")
