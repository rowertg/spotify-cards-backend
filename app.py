...
async def fetch_playlist_tracks(playlist_id: str, token: str, market: str = "") -> List[Dict[str, Any]]:
    # Erst ohne market versuchen
    ...
    async with httpx.AsyncClient(timeout=30) as client:
        while url:
            ...
            if r.status_code != 200:
                # Extra-Check: Playlist-Metadaten abrufen für bessere Fehlermeldung
...
def root():
    return ("OK: Spotify Cards Backend läuft.\n\n"
            "Nutzung:\n"
            "  /api/playlist.json?url=<playlist_url_oder_id>&market=DE\n"
            "  /api/playlist.csv?url=<playlist_url_oder_id>&market=DE\n"
...
async def debug_playlist(url: str = Query(...)):
    pid = extract_playlist_id(url)
    if not pid:
...
async def api_playlist_json(url: str = Query(...), market: str = Query("")):
    pid = extract_playlist_id(url)
...
async def api_playlist_csv(url: str = Query(...), market: str = Query("")):
    pid = extract_playlist_id(url)
    if not pid: raise HTTPException(status_code=400, detail="Ungültige Playlist-URL oder ID")
    token = await get_app_token()
    rows = await fetch_playlist_tracks(pid, token, market=market)
